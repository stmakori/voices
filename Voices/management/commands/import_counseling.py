...
class Command(BaseCommand):
    help = "Import counseling centers from JSON data"

    def add_arguments(self, parser):
        parser.add_argument("--file", default="counseling_data.json", help="Path to JSON file")
    ...
    def handle(self, *args, **options):
        ...
        entries = payload.get("counseling_centers", [])
        created = 0
        updated = 0
        skipped = 0
        ...
        for entry in entries:
            ...
            resource = Resource.objects.filter(name=name, resource_type="counseling").first()
            if resource is None:
                if options["dry_run"]:
                    created += 1
                    continue
        ...
        total = Resource.objects.filter(resource_type="counseling").count()
        self.stdout.write(self.style.SUCCESS(
            "Import complete:\n"
            f"  Created: {created}\n"
            f"  Updated: {updated}\n"
            f"  Skipped: {skipped}\n"
            f"  Counseling centers in database: {total}"
        ))
