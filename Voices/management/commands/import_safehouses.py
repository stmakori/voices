...
class Command(BaseCommand):
    help = "Import safe houses / rescue centers from JSON data"

    def add_arguments(self, parser):
        parser.add_argument(
            "--file",
            default="safehouse_data.json",
            help="Path to JSON file (default: safehouse_data.json)",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        entries = payload.get("safehouses", [])
        if not entries:
            self.stdout.write(self.style.WARNING("No safehouses found in input file."))
            return
        ...
        for entry in entries:
            ...
            if resource is None:
                ...
                Resource.objects.create(
                    ...
                    address=address,
                    phone=phone,
                    description="Safe house / rescue center.",
                    latitude=float(lat),
                    longitude=float(lon),
                    is_verified=True,
        ...
        total_safehouses = Resource.objects.filter(resource_type="safehouse").count()
        self.stdout.write(
            self.style.SUCCESS(
                "Import complete:\n"
                f"  Created: {created}\n"
                f"  Updated: {updated}\n"
                f"  Skipped: {skipped}\n"
                f"  Safe houses in database: {total_safehouses}"
            )
        )
